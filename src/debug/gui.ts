 ...
 */
export function createTuningGui(tuning: Tuning, onReset: () => void): GUI {
  ...
  pass.add(tuning.pass, "controlHoldTime", 0, 2, 0.05).name("出脚后保持控制 s");
  ...
  ai.add(tuning.ai, "autoSwitchCooldown", 0, 3, 0.05).name("自动切人冷却 s");
  ai.add(tuning.ai, "manualSwitchLock", 0, 6, 0.1).name("手动切人后锁定 s");
  ai.add(tuning.ai, "autoSwitchMargin", 0, 2, 0.05).name("自动切人迟滞 s（0=疯狂跳人）");
  ...
  ai.add(tuning.ai, "gkLineDepth", 0, 8, 0.1).name("门将出击距离 m");
  ai.add(tuning.ai, "gkSpeed", 1, 10, 0.1).name("门将移动速度 m/s");
  ai.add(tuning.ai, "gkReaction", 0, 1.5, 0.02).name("门将反应时间 s（0=无敌）");
  ai.add(tuning.ai, "gkReach", 0.3, 4, 0.05).name("门将站着的手长 m");
  ai.add(tuning.ai, "gkDiveReach", 0.5, 6, 0.1).name("鱼跃时的手长 m");
  ai.add(tuning.ai, "gkCatchHeight", 0.5, 5, 0.1).name("能接多高的球 m");
  ai.add(tuning.ai, "gkDiveSpeed", 2, 20, 0.5).name("鱼跃速度 m/s");
  ai.add(tuning.ai, "gkDiveWindow", 0.1, 1.5, 0.05).name("鱼跃窗口 s");
  ai.add(tuning.ai, "gkDiveRecovery", 0, 3, 0.05).name("鱼跃后趴地 s");
  ai.add(tuning.ai, "gkDiveTrigger", 0, 6, 0.1).name("多远才鱼跃 m");
  ai.add(tuning.ai, "gkClearDelay", 0, 3, 0.05).name("门将握球时间 s");
  ai.add(tuning.ai, "gkClearLoft", 0, 60, 1).name("门将开球仰角 度");
  ai.add(tuning.ai, "gkClearPower", 8, 45, 0.5).name("门将开球力度 m/s");
